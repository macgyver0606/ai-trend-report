...
class NaverNewsServiceTests(unittest.TestCase):
    def test_search_filters_period_removes_duplicates_and_strips_tags(self):
        payload = {
            "items": [
                ...
                {"title": "기간 밖 기사", "description": "기간 밖", "pubDate": "Mon, 01 Jun 2026 08:00:00 +0900", "originallink": "https://news.example.com/2", "link": "https://n.news.naver.com/2"},
            ]
        }
        response = MagicMock()
        response.read.return_value = json.dumps(payload).encode("utf-8")
        response.__enter__.return_value = response
        request = AnalysisRequest(keyword="AI", industry="반도체", start_date=date(2026, 7, 1), end_date=date(2026, 7, 25))
